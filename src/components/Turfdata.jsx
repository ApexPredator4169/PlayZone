...
export const Turfdata = (prop) => {
  const { turf, selectedDistrict } = prop;
  ...
  useEffect(() => {
    setLoading(true);
    const turfRef = collection(db, turf);
    const getData = async () => {
      try {
        const q = query(turfRef, where("district", "==", selectedDistrict));
        const turfData = await getDocs(q);
